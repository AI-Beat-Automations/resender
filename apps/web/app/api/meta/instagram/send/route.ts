...
export async function POST(request: NextRequest) {
  ...
  // Gate de entitlement (ADR 0003), igual que en Messenger: con la cuota del
  ...
  // token vencido nuestro ni por la ventana de 24 h de Instagram. Los replays
  // idempotentes ya devolvieron antes de llegar acá, así que tampoco suman.
  // Best-effort: un fallo del contador no puede hacer fallar un mensaje que
  // Meta ya entregó.
  if (metaResult.ok) {
    try {
