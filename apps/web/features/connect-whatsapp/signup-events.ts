...
// llegan por acá **no son autoritativos**. `lib/whatsapp.ts` los trata como
// pista y los confirma contra Graph con el token recién canjeado, así que un
// `waba_id` inyectado muere ahí aunque el origen nos engañara.
...
// servidor, nunca fuente de verdad (ver `lib/whatsapp.ts`).
...
//
// - `finished`: el único camino feliz de este slice (flujo estándar Cloud API).
// - `finished-without-number`: terminó, pero sin número. `FINISH_ONLY_WABA`, y
//   también un `FINISH` al que le falta el `phone_number_id`.
// - `unsupported-flow`: terminó bien, pero por una variante que todavía no
//   soportamos (Coexistence, migración OBO, grant-only). No es un fallo nuestro
//   ni del usuario, y decir «hubo un error» sería mentirle.
...
// Cierra el flujo `FINISH*` que sí sabemos terminar hoy. El resto se nombra
// aparte para poder explicarle al usuario qué hizo, en vez de fallar en
// genérico.
const SUPPORTED_FINISH_EVENT = "FINISH"
...
export function readWhatsappSignupEvent(
  ...
  if (event === SUPPORTED_FINISH_EVENT) {
    ...
    // Un `FINISH` sin número es, en la práctica, un `FINISH_ONLY_WABA`: el
    // usuario saltó el alta del teléfono. Se cuenta igual porque el remedio es
    // el mismo.
    if (!phoneNumberId) return { kind: "finished-without-number" }
  ...
  // `FINISH_GRANT_ONLY_API_ACCESS` y cualquier `FINISH*` que Meta agregue
  // después. Se reconocen por prefijo para que un valor nuevo llegue como «no
  // soportado todavía» —que es la verdad— y no como un mensaje ignorado que deja
  // el botón girando.
...
// Los `FINISH*` que no son el flujo estándar. Cada uno dice **qué** completó el
// usuario y que ese camino todavía no está soportado: terminó bien, no falló
// nada, y un «hubo un error» acá sería mentirle y mandarlo a reintentar en
// bucle.
function describeUnsupportedFlow(event: string): string {
  switch (event) {
    case "FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING":
      return "El número que elegiste sigue en uso en la app de WhatsApp Business. Ese flujo (Coexistence) todavía no está soportado en Resender: conecta un número que no esté en la app, o escríbenos a info@resender.dev."
    ...
    case "FINISH_GRANT_ONLY_API_ACCESS":
      return "Solo diste acceso a la API, sin conectar un número. Vuelve a lanzar la conexión y completa el flujo estándar para elegir el teléfono."
    default:
