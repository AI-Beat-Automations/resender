...
import { toAttachmentDisplay, whatsappMediaUrl } from "./message-media"
...
describe("toAttachmentDisplay", () => {
  it("devuelve null cuando el mensaje no trae adjunto", () => {
    expect(toAttachmentDisplay(null, es)).toBeNull()
  })

  it("mapea imagen y sticker con URL a preview de imagen", () => {
    expect(
      toAttachmentDisplay({ type: "image", url: CDN, meta: null }, es)
  ...
  it("mapea video, reel e ig_reel a preview de video", () => {
    for (const type of ["video", "reel", "ig_reel"]) {
      expect(toAttachmentDisplay({ type, url: CDN, meta: null }, es)).toEqual({
        kind: "video",
        url: CDN,
      })
    }
  ...
  it("mapea audio a preview de audio", () => {
    expect(
      toAttachmentDisplay({ type: "audio", url: CDN, meta: null }, es)
  ...
  it("usa el type a secas cuando el meta no aporta nada", () => {
    expect(
      toAttachmentDisplay({ type: "fallback", url: null, meta: {} }, es)
    ).toEqual({ kind: "row", label: "fallback", url: null })
    expect(
      toAttachmentDisplay({ type: "template", url: null, meta: null }, es)
  ...
  it("degrada a fila la imagen sin URL usable", () => {
    ...
    expect(
      toAttachmentDisplay({ type: "image", url: null, meta: null }, es)
    ).toEqual({ kind: "row", label: "image", url: null })
    expect(
      toAttachmentDisplay(
        { type: "image", url: "http://inseguro", meta: null },
        es
      )
    ).toEqual({ kind: "row", label: "image", url: null })
  })
  ...
  it("solo mapea el adjunto: el texto del mensaje no es asunto suyo", () => {
    ...
    expect(
      toAttachmentDisplay(
        { type: "image", url: CDN, meta: { title: "foto" } },
        es
      )
    ).toEqual({ kind: "image", url: CDN })
  })
...
describe("adjuntos de WhatsApp", () => {
  it("pinta como fila los seis types que suma la 0017", () => {
    ...
    for (const type of [
      ...
      expect(
        toAttachmentDisplay({ type, url: MEDIA_ROUTE, meta: null }, es)
...
describe("ATTACHMENT_STATUS_COPY", () => {
  it("le da copy propio a los cinco estados", () => {
    ...
    expect(es.inbox.attachmentStatus).toEqual({
      pending: "descargando…",
    ...
    // Cinco estados, cinco textos distintos, en los dos idiomas: colapsar dos
    // deja a soporte sin poder distinguir un bug nuestro de un límite de Meta.
    for (const dict of [es, en]) {
      expect(new Set(Object.values(dict.inbox.attachmentStatus)).size).toBe(5)
    }
  })
  ...
  it("el estado gana sobre la URL: nada de previews rotos", () => {
    for (const status of [
      ...
      expect(
        ...
        kind: "row",
        label: `image · ${es.inbox.attachmentStatus[status]}`,
        url: null,
      })
    }
  })
  ...
  it("no toca a Messenger ni a Instagram, que no informan estado", () => {
    expect(
      toAttachmentDisplay(
        { type: "image", url: CDN, meta: null, status: null },
        es
      )
    ).toEqual({ kind: "image", url: CDN })
  })
