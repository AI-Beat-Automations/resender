...
Quién está detrás de la [Sesion] y a qué tenant pertenece lo que ve: `{ tenantId, userId, clientAccountId | null }` (`lib/clients/actor.ts`, `resolveActor(session)`). Para el [Padre] `tenantId === userId` y `clientAccountId` es nulo: nada cambia para él. Para un [Cliente] activo, `tenantId` es el del padre y `clientAccountId` es lo que distingue lo suyo. Se lee **vivo contra la base en cada request y nunca de la sesión**, con la misma doctrina que el [Gate de acceso]: fail-closed, sin fila no hay actor. El layout de `(product)` lo resuelve primero (sesión → actor → gates → render) y todo lo que se oculta o filtra por rol —sidebar, pestañas de Ajustes, aviso de cuota, más adelante Conexiones e Inbox— se decide a partir de él con funciones puras por actor (`productNavItems`, `settingsTabsFor`). Un user con fila de cliente todavía `pending` no tiene actor: ve la [Cuenta restringida] sin padre nombrado.
